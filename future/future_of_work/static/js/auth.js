...
regBtn && (regBtn.onclick = () => {window.location = onboardingUrl;});
...
loginForm && loginForm.addEventListener('submit', function(e){
  ...
  if(u) {
    ...
    window.location = window.dashboardUrl; return;
  }
  loginError.textContent='Invalid email or password.'; loginError.classList.remove('hidden');
});
// Redirect if already logged in
window.addEventListener('DOMContentLoaded',()=>{
  if(localStorage.getItem('sessionUser')) window.location = window.dashboardUrl;
});
